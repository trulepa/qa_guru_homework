...
class TestSuite:
    ...
    def __init__(self, variables):
        self.url = variables.url # "https://qa-guru.github.io/one-page-form/text-box.html"
        self.driver = webdriver.Chrome()
        self.locator = variables.locator
        self.result_line_name = variables.result_line_name
    ...
    def search_fill_fields(self, field_locator, send_keys):
        field = self.driver.find_element(By.ID, field_locator)
        field.send_keys(send_keys)

    # Находим кнопку по ее ID и кликаем
    def click_submit(self, submit_locator):
        submit_button = self.driver.find_element(By.ID, submit_locator)
        submit_button.click()
        time.sleep(2)  # Пауза, чтобы визуально заметить открытие

    # Находим блок с отправленными данными
    def search_result_box(self, result_box_locator):
        result_box = self.driver.find_element(By.ID, result_box_locator)
    ...
    def empty_full_name(self):
        try:
            empty_test.set_up()
            empty_test.search_fill_fields(self.locator["full_name_locator"], "")
            empty_test.click_submit(self.locator["submit_button_locator"])
            result_box = empty_test.search_result_box(self.locator["result_box_locator"])
...
empty_test = TestSuite(variables)
